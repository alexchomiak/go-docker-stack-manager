...
func StopComposeStack(c *fiber.Ctx) error {
	...
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"errors": "Error creating temporary file for Stack Removal",
		})
	}
	defer os.Remove(file.Name())

	cmd := exec.Command("docker-compose", "-f", file.Name(), "-p", stack.StackID, "down")
	...
	return c.JSON(fiber.Map{
		"message": "Stack Torn down successfully",
	})
}
